...
class C_BIoUTracker(object):
    def __init__(self, args, frame_rate=30):
        ...
        self.det_thresh = args.conf_thresh + 0.15
        self.buffer_size = int(frame_rate / 30.0 * args.track_buffer)*10
    ...
    def update(self, output_results, img, ori_img):
        ...
        dists = buffered_iou_distance(tracklet_pool, detections, level=2)#调参
        
        matches, u_track, u_detection = linear_assignment(dists, thresh=0.85)#调参
        ...
        dists = buffered_iou_distance(r_tracked_tracklets, detections_second, level=2)#调参

        matches, u_track, u_detection_second = linear_assignment(dists, thresh=0.5)#调参
        for itracked, idet in matches:
        ...
        dists = buffered_iou_distance(unconfirmed, detections, level=1)#调参
       
        matches, u_unconfirmed, u_detection = linear_assignment(dists, thresh=0.85)#调参
